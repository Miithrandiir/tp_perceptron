...
void Stats::getPlorErrorByLearningStep(int max_exemple) {
    ...
    for(int w=0;w<=max_exemple;w++) {
        std::cout << "[INFO] Generation du plot 'errorbylearning_step_'" << std::endl;
        ...
        plt::close();
        std::cout << "[INFO] FIN Generation du plot 'errorbylearning_step_'" << std::endl;
    }
}
